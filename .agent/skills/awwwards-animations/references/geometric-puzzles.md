...
const TRIANGLE_PIECES = [
  {
    id: "A",
    tri: "M 0,173.2 L 50,86.6 L 100,173.2 Z",
    sq: "M 0,0 L 100,0 L 100,86.6 L 0,86.6 Z",
  },
  {
    id: "B",
    tri: "M 50,86.6 L 100,0 L 150,86.6 Z",
    sq: "M 100,0 L 200,0 L 200,86.6 L 100,86.6 Z",
  },
  {
    id: "C",
...
export function Tangram() {
  const svgRef = useRef<SVGSVGElement>(null);
  const [activeTarget, setActiveTarget] = useState<keyof typeof TARGETS | null>(
    null,
  );
  ...
  for (let i = 0; i < 10; i++) {
    ...
    tiles.push({
      ...
      vertices: [
        ...
        [
          cx + (Math.cos(mid) * radius) / PHI,
          cy + (Math.sin(mid) * radius) / PHI,
        ],
...
export function GeometricTransform({
  type = "rotate",
}: {
  type?: TransformType;
}) {
