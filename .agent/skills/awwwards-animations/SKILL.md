...
export function GlitchText({ text }: { text: string }) {
  ...
  return (
    <div ref={ref} className="relative font-mono text-5xl font-black">
      ...
      <span
        className="g-layer absolute inset-0 text-cyan-400 mix-blend-multiply"
        aria-hidden
      >
        {text}
      </span>
      <span
        className="g-layer absolute inset-0 text-red-400 mix-blend-multiply"
        aria-hidden
      >
        {text}
      </span>
...
export function GeometricDissection() {
  ...
  return (
    <div className="flex flex-col items-center gap-4">
      ...
      <button
        onClick={morph}
        className="px-6 py-2 bg-white text-black font-mono text-sm"
      >
        {isSq ? "△" : "□"}
      </button>
    </div>
